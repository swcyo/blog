...
data<-read.csv("/Users/mac/Documents/GitHub/blog/content/course/2022-11-20-two-way-repeated-measures-anova/data.csv")
knitr::kable(data,caption = "需要的数据格式")
```
...
```{r echo=FALSE}
knitr::kable(data,rownames = 1,caption = "新增1列id")
```
...
```{r echo=FALSE}
knitr::kable(data2,caption = "短数据转换为长数据")
```
...
```{r echo=FALSE}
knitr::kable(data3,caption = "各组统计学描述")
```
...
```{r echo=FALSE}
knitr::kable(Pairwise,caption = "土耳其检验")
```
